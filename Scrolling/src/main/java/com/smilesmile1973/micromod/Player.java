...
package com.smilesmile1973.micromod;
...
public class Player implements Runnable {
		...
		try {
			...
			try {
				...
				while( playing && ( samplePos < duration || loop ) ) {
					...
					notifyPlayerListeners(mixBuf,mixEnd);
					samplePos += count;
				}
				audioLine.drain();
			} finally {
	...
	public void notifyPlayerListeners(int[] data,int length){
		for (PlayerListener listener : playerListeners) {
			listener.processFrequency(data,length);
		}
	}
